...
anomalies["Sales"] = pd.to_numeric(
    anomalies["Sales"],
    errors="coerce"
)

anomalies["RollingMean"] = pd.to_numeric(
    anomalies["RollingMean"],
    errors="coerce"
)

anomalies["RollingStd"] = pd.to_numeric(
    anomalies["RollingStd"],
    errors="coerce"
)

anomalies["ZScore"] = pd.to_numeric(
    anomalies["ZScore"],
    errors="coerce"
)

# Convert True/False text to boolean
anomalies["Isolation"] = anomalies["Isolation"].astype(str).str.lower() == "true"
anomalies["Rolling_Z"] = anomalies["Rolling_Z"].astype(str).str.lower() == "true"

# Remove any invalid rows
anomalies = anomalies.dropna(subset=["Order Date", "Sales"])
import csv
...
# =====================================================
    ...
    iso = anomalies[anomalies["Isolation"]]
    ...
    z = anomalies[anomalies["Rolling_Z"] == 1]
    ...
    ax2.set_title("Rolling Z-Score Anomaly Detection")
    ax2.set_xlabel("Date")
...
# =====================================================
    ...
    st.subheader("Product Demand Cluster Chart")

    fig, ax = plt.subplots(figsize=(10,7))

    colors = {
    "High Volume, Stable Demand":"green",
    "Growing Demand":"blue",
    "Low Volume, High Volatility":"orange",
    ...
    for segment in clusters["Demand Segment"].unique():
        ...
        temp = clusters[
        clusters["Demand Segment"] == segment
    ]
